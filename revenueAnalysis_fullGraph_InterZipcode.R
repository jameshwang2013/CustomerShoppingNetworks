...
###############################################################################
###### ANALYSIS: please run functions in CALCULATIONS first. ##################
###############################################################################

###### full graph regression analysis
...
### export results
for (i in names(zip_code_results)) {
  
  zip_results_regressions <- zip_code_results[[i]]$results_regression
  exportResults(zip_results_regressions, i)
}


###############################################################################
###### PREDICTION: Run ANALYSIS before to identify features to use. ###########
###############################################################################

###### Calculate predictions for each revenue type
full_features <- full_graph_results$results_for_corr_plots$data_revenue_logY_trunc0
customer_ids <- full_graph_results$idx
### Individual Revenue
...
###### combine customer predictions, export file
...
###############################################################################
###### CALCULATIONS: Run these functions before ANALYSIS ######################
###############################################################################

calculateRevenueResults <- function(nodes, links, g) {
  ...
  return(list(indices = idx,
              results_for_corr_plots = results_for_corr_plots,
